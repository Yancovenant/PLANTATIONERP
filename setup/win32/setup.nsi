...
!ifndef SERVICENAME
	!define SERVICENAME 'inphms-server-${VERSION}'
!endif

!ifndef TOOLSDIR
	!define TOOLSDIR 'c:\inphmsbuild'
!endif

!define PRODUCT_NAME "Inphms"
...
!define UNINSTALL_REGISTRY_KEY_SERVER "${UNINSTALL_BASE_REGISTRY_KEY}\Inphms Server ${VERSION}"
...
OutFile "${TOOLSDIR}\server\inphms_setup_${VERSION}.exe"
...
!define MUI_ICON "${PIXMAPS_PATH}\inphms-icon.ico"

!define MUI_WELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\inphms-intro.bmp"
!define MUI_UNWELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\inphms-intro.bmp"
!define MUI_HEADERIMAGE
!define MUI_HEADERIMAGE_BITMAP "${PIXMAPS_PATH}\inphms-slogan.bmp"
...
!define MUI_FINISHPAGE_LINK_LOCATION "https://www.inphms.com/page/contactus"
...
; English
LangString DESC_Inphms_Server ${LANG_ENGLISH} "Install the Inphms Server with all the Inphms standard modules."
LangString DESC_PostgreSQL ${LANG_ENGLISH} "Install the PostgreSQL RDBMS used by Inphms."
LangString DESC_FinishPage_Link ${LANG_ENGLISH} "Contact Inphms for Partnership and/or Support"
LangString DESC_AtLeastOneComponent ${LANG_ENGLISH} "You have to choose at least one component"
LangString DESC_CanNotInstallPostgreSQL ${LANG_ENGLISH} "You can not install the PostgreSQL database without the Inphms Server"
...
LangString Profile_AllInOne ${LANG_ENGLISH} "Inphms Server And PostgreSQL Server"
LangString Profile_Server ${LANG_ENGLISH} "Inphms Server Only"
LangString Profile_IOT ${LANG_ENGLISH} "Inphms IoT"
LangString TITLE_Inphms_Server ${LANG_ENGLISH} "Inphms Server"
LangString TITLE_PostgreSQL ${LANG_ENGLISH} "PostgreSQL Database"
LangString TITLE_IOT ${LANG_ENGLISH} "Inphms IoT"
...
Section $(TITLE_Inphms_Server) SectionInphms_Server
    SectionIn 1 2 3
    ...
    # If there is a previous install of the Inphms Server, keep the login/password from the config file
    WriteIniStr "$INSTDIR\server\inphms.conf" "options" "db_host" $TextPostgreSQLHostname
    WriteIniStr "$INSTDIR\server\inphms.conf" "options" "db_user" $TextPostgreSQLUsername
    WriteIniStr "$INSTDIR\server\inphms.conf" "options" "db_password" $TextPostgreSQLPassword
    WriteIniStr "$INSTDIR\server\inphms.conf" "options" "db_port" $TextPostgreSQLPort
    # Fix the addons path
    WriteIniStr "$INSTDIR\server\inphms.conf" "options" "addons_path" "$INSTDIR\server\inphms\addons"
    WriteIniStr "$INSTDIR\server\inphms.conf" "options" "bin_path" "$INSTDIR\thirdparty"
    # Set data_dir
    WriteIniStr "$INSTDIR\server\inphms.conf" "options" "data_dir" "$INSTDIR\sessions"
    ...
    ${If} $HasPostgreSQL == 0
        WriteIniStr "$INSTDIR\server\inphms.conf" "options" "pg_path" "$INSTDIR\PostgreSQL\bin"
    ${EndIf}

    # Productivity Apps
    WriteIniStr "$INSTDIR\server\inphms.conf" "options" "default_productivity_apps" "True"
    DetailPrint "Installing Windows service"
    nsExec::ExecTOLog '"$INSTDIR\python\python.exe" "$INSTDIR\server\inphms-bin" --stop-after-init -c "$INSTDIR\server\inphms.conf" --logfile "$INSTDIR\server\inphms.log" -s'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" install ${SERVICENAME} "$INSTDIR\python\python.exe"'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} AppDirectory "$\"$INSTDIR\python$\""'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} AppParameters "\"$INSTDIR\server\inphms-bin\" -c "\"$INSTDIR\server\inphms.conf\"'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} ObjectName "LOCALSERVICE"'
    AccessControl::GrantOnFile  "$INSTDIR" "LOCALSERVICE" "FullAccess"

    Call RestartInphmsService
SectionEnd
...
Section $(TITLE_PostgreSQL) SectionPostgreSQL
    ...
    ExecWait '"$TEMP\$postgresql_exe_filename" \
        ...
        --servicename "PostgreSQL_For_Inphms" \
...
Section $(TITLE_IOT) IOT
    ...
    WriteIniStr "$INSTDIR\server\inphms.conf" "options" "server_wide_modules" "web,hw_posbox_homepage,hw_drivers"
    WriteIniStr "$INSTDIR\server\inphms.conf" "options" "list_db" "False"
    WriteIniStr "$INSTDIR\server\inphms.conf" "options" "max_cron_threads" "0"
    nsExec::ExecToStack '"$INSTDIR\python\python.exe" "$INSTDIR\server\inphms-bin" genproxytoken'
    pop $0
    pop $ProxyTokenPwd
SectionEnd
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
    !insertmacro MUI_DESCRIPTION_TEXT ${SectionInphms_Server} $(DESC_Inphms_Server)
...
Function .onInit
    ...
    ${If} $previous_install_dir == ""
        StrCpy $INSTDIR "$PROGRAMFILES64\Inphms ${VERSION}"
...
Function ShowProxyTokenDialogPage
    ...
    doProxyToken:
        ...
        ${NSD_CreateLabel} 0 0 100% 25% "Here is your access token for the Inphms IOT, please write it down in a safe place, you will need it to configure the IOT"
        Pop $ProxyTokenLabel
...
Function ComponentLeave
    SectionGetFlags ${SectionInphms_Server} $0
...
Function RestartInphmsService
    DetailPrint "Restarting Inphms Service"
